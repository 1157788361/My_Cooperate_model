...
def construct_input(segmentation, image=None, num_classes=None, apply_softmax=True, temperature=2, is_labelmap=False, smooth_label=False, shuffle=False, use_gpu=True):
    ...
    if not is_labelmap:
        ...
        #                  label 中 : 0,1,2,3         # label 中 : 0,1
        # 以下操作的目的是将 N X H X W 的 gt_label 转换为 N X C X H X W。
        flatten_y = segmentation.view(batch_size * h * w, 1) # 将 预测的 y 拉平成： batch_size * h * w  X 1
        y_onehot = torch.zeros(batch_size * h * w, num_classes, dtype=torch.float32, device=device) # 形状为 ： batch_size * h * w  X num_classes
        # 就是将 预测的 y 变为 onehot 形式。
        y_onehot.scatter_(1, flatten_y, 1) # scatter() 将输入 1 按照 flatten_y 重新分配 到 y_onehot 中。
        # scatter() 将输入 src 按照 index 重新分配 到 tensor_A 中。
        # tensor_A.scatter(dim, index, src)
        # input = torch.rand(2, 5)
        # index = torch.tensor([[0, 1, 2, 0, 0], [2, 0, 0, 1, 2]])
        # output = torch.zeros(3, 5).scatter_(0, index, input)
        # 第一行index=[0,1,2,0,0]：
        #      [[0.62 , 0.0344 , 0.064 , 0.808 , 0.111 ],     -->      [[0.62 ,     0 ,     0 , 0.808 , 0.111 ],
        #       [0.75 , 0.32   , 0.213 , 0.578 , 0.636]]                [ 0   , 0.0344,     0 , 0     ,     0 ]
        #                                                               [ 0   ,     0 , 0.064 , 0     ,     0 ]]
        # 解释： 当 dim = 0 时，也即 按行进行遍历，且 按照 index 中的 值改变原 行的 维度。

        # 第一行index为[3, 2, 0, 1]
        # 当 dim = 1 ,且 index= [[3, 2, 0, 1], [1, 2, 0, 3]]
        #        [[0.62 , 0.0344 , 0.064 , 0.808 , 0.111 ],     -->      [[ 0.064 , 0.808 , 0.0344 ,  0.62 , 0 ],
        #         [0.75 , 0.32   , 0.213 , 0.578 , 0.636]]                [ 0.213 , 0.75  ,   0.32 , 0.578 , 0 ]
        #                                                                 [     0 ,     0 ,      0 ,    0  , 0 ]]

        y_onehot = y_onehot.view(batch_size, h, w, num_classes)
        y_onehot = y_onehot.permute(0, 3, 1, 2) # 还原到  N X C X H X W

        y_onehot.requires_grad = False

        if smooth_label: # False
            # add noise to labels
