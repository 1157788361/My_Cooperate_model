...
class runningScore(object):
    ...
    def _fast_hist(self, label_true, label_pred, n_class):
        ...
        hist = np.bincount(
            n_class * label_true[mask].astype(int) + # label_true[mask] 的结果为 一维。且 index 为 false 的 不会显示。
            label_pred[mask], minlength=n_class ** 2).reshape(n_class, n_class)
        # 如 n_class =3 label=0,1,2 ，size = 5X5X5
        # n_class * label_true[mask].astype(int) ，shape = 125,
        # label_pred[mask] ，shape = 125,
        # n_class * label_true[mask].astype(int)+label_pred[mask] ，shape = 125.
        # label = 0 ,1(错检为背景:1->0),2(错检为背景:2->0),3(漏检label_1:0->3),4(预测label_1正确:1->3),5(错检为label_1:2->3),6(漏检label_2:0->6),7(错检为label_2:1->6),8(预测label_2正确:2->6)    # 0,1,2(预测)   与 0,3,6(真实)
        # minlength = 9
        # np.bincount(...) 会统计 label 出现次数 且按原 label 顺序排序,label 最大为 len(label) = 9 即 label = 8。
        # 也即 会统计 0,1,2,3,4,5,6,7,8 出现的次数
    ...
    def get_scores(self):
        ...
        # 对角线为预测正确的。
        acc = np.diag(hist).sum() / hist.sum() # np.diag(hist) 未指定dim,输入为二维数组时 ，结果为对角线元素

        # 每类的预测 acc 情况
        acc_cls = np.diag(hist) / hist.sum(axis=1) # 移动 dim=1
        # 平均的预测 acc 情况
        acc_cls = np.nanmean(acc_cls)

        #   label_1 预测正确 / (预测出的label_1 + label_1的真实值 - label_1 预测正确)  # 结合混淆矩阵。
        iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist)) # 3 X 1
        mean_iu = np.nanmean(iu) # 在求和、均值时 忽略nan

        freq = hist.sum(axis=1) / hist.sum() # 混淆矩阵中的 ： Tlabel_1 + Flabel_1  。 预测的每个label 预测出现的频率。
        # 3 X 1
        fwavacc = (freq[freq > 0] * iu[freq > 0]).sum() # 经过 freq 加权的 acc
        cls_iu = dict(zip(range(self.n_classes), iu)) # 每个 class 对应的 iou
